/**
 * Anthropic Claude Code — project `.claude/settings.json` PostToolUse (Write|Edit).
 * Tab argv: reserve `CLAUDE_CODE_HOOK_ARG_TAB_RESERVED` in constants (not wired).
 */
...
import {
  ...
  claudeCodeSettingsContainsOurs,
...
function runDoctor(ctx: EditorDoctorContext): void {
  ...
  const hp = join(repoRoot, '.claude', 'hooks', HOOK_SCRIPT_NAME);

  if (!existsSync(sp)) console.warn(`[warn] [claude-code] Missing ${sp} — run init`);
  else {
    const raw = readFileSync(sp, 'utf8');
    if (claudeCodeSettingsContainsOurs(raw)) {
      console.log('[ok] [claude-code] settings.json contains aicode-ratio hook command');
    } else {
      console.warn(
        `[warn] [claude-code] ${sp} exists but has no aicode-ratio PostToolUse entry — run init`,
      );
    }
  }

  if (existsSync(hp)) console.log(`[ok] [claude-code] Hook script present: ${hp}`);
  else console.warn(`[warn] [claude-code] Missing hook script ${hp} — run init`);
}
...
export const claudeCodeAdapter: EditorAdapter = {
  ...
  install(ctx: EditorInstallContext): void {
    const base = join(ctx.repoRoot, '.claude');
    ...
    const merged = mergeClaudeCodeSettings(prev);
    writeFileSync(sp, `${JSON.stringify(merged, null, 2)}\n`, 'utf8');

    installAicodeRatioReportCommand(join(ctx.repoRoot, '.claude', 'commands'));
  },
  ...
  uninstall(ctx: EditorUninstallContext): string[] {
    const lines: string[] = [];
    const cmdRm = tryRemoveAicodeRatioReportCommand(join(ctx.repoRoot, '.claude', 'commands'));
    if (cmdRm) lines.push(`Removed ${cmdRm}`);

    const sp = settingsAbs(ctx.repoRoot);
    if (!existsSync(sp)) return lines;
    const prev = readFileSync(sp, 'utf8');
    writeFileSync(sp, stripClaudeCodeSettingsRaw(prev), 'utf8');
