...
- 为所选编辑器写入 **`aicode-ratio-report`** slash 命令（Markdown）：`.cursor/commands/`、`.claude/commands/`、`.codebuddy/commands/`、`.qoder/commands/`、`.codex/commands/` 之一或多项；内容要求 Agent **不得**在未确认 `--since`/`--until` 前执行 `report`（无预填默认月；占位符命令，需从用户消息解析或交互询问后再替换执行）；**若始终无法识别或确认时间范围，则不得生成报表**。说明中约定：像「本月」且未说「整月 / 到月底」时，按 **月初至「今天」（UTC）** 换算半开区间（`until` 为「今天」UTC 日期的次日），勿默认用「下月 1 日」当整月右边界。
